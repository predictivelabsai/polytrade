...
class HeaderCachingJwkClient:
    """HTTPS JWKS cache with header TTLs and one refresh on an unknown key ID."""
...
class TokenVerifier:
    def __init__(self, settings: AgentSettings):
        ...
        self._clients = {
            key: HeaderCachingJwkClient(value.jwks_url)
    ...
    async def verify(
        ...
        issuer_value = unverified.get("iss")
        issuer = self.issuers.get(issuer_value)
        if issuer is None:
        ...
        try:
            signing_key = await asyncio.to_thread(
                self._clients[issuer.issuer].get_signing_key_from_jwt,
                token,
            )
            claims = jwt.decode(
                token,
        ...
        # PyJWT already validates exp/nbf against wall-clock time. Check the
        # remaining claim relationships using a wall-clock timestamp here.
        now = int(time())
